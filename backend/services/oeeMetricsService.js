...
async function writeOEEToInfluxDB(metrics) {
    try {
        ...
        const point = new Point('oee_metrics')
            ...
            .floatField('availability', metrics.oeeAsPercent ? metrics.availability * 100 : metrics.availability)
            .floatField('performance', metrics.oeeAsPercent ? metrics.performance * 100 : metrics.performance)
            .floatField('quality', metrics.oeeAsPercent ? metrics.quality * 100 : metrics.quality)
        ...
        oeeLogger.info(`Successfully wrote OEE metrics for machine ID: ${metrics.processData.machineId || 'undefined'} to InfluxDB.`);
