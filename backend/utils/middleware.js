...
function validateOEEData(req, res, next) {
    ...
    const { plannedProduction, runtime, actualPerformance, targetPerformance, goodProducts, totalProduction } = req.body;

    // Check if any of the required fields are missing or falsy
    if (!plannedProduction || !runtime || !actualPerformance || !targetPerformance || !goodProducts || !totalProduction) {
...
function handleErrors(err, req, res, next) {
    ...
    res.status(500).json({ message: 'Internal server error', error: err.message }); // Send a 500 response with the error message
}
